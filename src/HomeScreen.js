    ...
    fetch('https://fake-rest-api-nodejsa.herokuapp.com/user')
      ...
      .then((json) => {
        ...
        for (var i = 0; i < json.length; i++) {
          totallmoney +=
            json[i]['borrow'] - (json[i]['borrow'] * json[i]['ratio']) / 10;
  ...
  render() {
    ...
    const data1 = [
      ...
      },
      {
        key: 3,
        amount:
          this.state.data['authorizedcapital'] -
          (this.state.data['investmentmoney'] + this.state.totalloan),
        title: 'Tiền rảnh',
        svg: {fill: '#c61aff'},
      },
    ];
    ...
    return (
      <ScrollView style={homestyle.body}>
        ...
        <TouchableOpacity style={{position: "absolute", right: 12, top: 12}} onPress={()=>this.getApi()}>
        ...
        <View
          ...
          <View style={homestyle.moneybox}>
            <Text style={{fontSize: 22, color: '#FFFFFF'}}>
              {formatCurrency(data.authorizedcapital)}
            </Text>
            <Text style={{fontSize: 14, color: '#7B7F9E', marginTop: 12}}>
              TỔNG QUỸ TIỀN MẶT
            </Text>
          </View>
          <View style={homestyle.moneybox}>
            <Text style={{fontSize: 22, color: '#FFFFFF'}}>
              {this.state.totalcontract}
            </Text>
            <Text style={{fontSize: 14, color: '#7B7F9E', marginTop: 12}}>
        ...
        <View
          ...
          <View style={homestyle.moneybox}>
            <Text style={{fontSize: 22, color: '#FFFFFF'}}>
              {formatCurrency(this.state.totalloan)}
            </Text>
            <Text style={{fontSize: 14, color: '#7B7F9E', marginTop: 12}}>
              TIỀN ĐANG CHO VAY
            </Text>
          ...
          <View style={homestyle.moneybox}>
            <Text style={{fontSize: 22, color: '#FFFFFF'}}>
              {formatCurrency(this.state.Totalloanamount)}
            </Text>
            <Text style={{fontSize: 14, color: '#7B7F9E', marginTop: 12}}>
              LÃI XUẤT CHO VAY
            </Text>
        ...
        <View
          ...
          <View style={homestyle.moneybox}>
            <Text style={{fontSize: 22, color: '#FFFFFF'}}>
              {formatCurrency(this.state.data['loan'])}
            </Text>
            <Text style={{fontSize: 14, color: '#7B7F9E', marginTop: 12}}>
              TIỀN ĐANG GÓP VỐN
            </Text>
          ...
          <View style={homestyle.moneybox}>
            <Text style={{fontSize: 22, color: '#FFFFFF'}}>
              {formatCurrency(this.state.totalinterestrate)}
            </Text>
            <Text style={{fontSize: 14, color: '#7B7F9E', marginTop: 12}}>
              LÃI GÓP VỐN
            </Text>
        ...
        <View style={homestyle.menu}>
          ...
          <View>
            <TouchableOpacity
              ...
              onPress={() => this.props.navigation.navigate('BorrowerScreen')}>
          ...
          <View
            ...
            {data1.map((item) => (
              <View
                ...
                <Text style={[homestyle.text3, {marginLeft: 4}]}>
                  {item.title}:{' '}
                </Text>
                <Text style={homestyle.text3}>
